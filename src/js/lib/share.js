...
Array.from( document.querySelectorAll( cfgShare.selector ) ).forEach(share => {
  ...
  // enable app links
  if (navigator.share) {
    Array.from( share.querySelectorAll(`[hidden]:has(a[href$="${ cfgShare.appHash }"])`) ).forEach(applink => applink.removeAttribute('hidden'));
  }

  // activate hidden print
  Array.from( share.querySelectorAll(`[hidden]:has(a[href$="${ cfgShare.printHash }"])`) ).forEach(applink => applink.removeAttribute('hidden'));
